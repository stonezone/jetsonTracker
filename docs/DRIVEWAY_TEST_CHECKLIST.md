...
**`tracking`**, and **`gps`** sections. The iOS app shows owner / state / locked / conf.
...
5. **GPS fix** — base Wio on **battery** (off the Orin USB rail) outside until it has sats;
...
6. **CALIBRATE** (iOS Calibrate tab) — location lock (averaged base fix) → level →
   **heading aimed at mom's house (328 ft)** — the best target you have. (The gate at 164 ft
   ...
   confirm → `authority.calibration_valid: true`.
